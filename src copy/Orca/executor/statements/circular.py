...
class CircularBlock():
    ...
    async def execute(self, content, all_states=None):
        # flag, type = self.validate(content)
        flag, type = True, "for"
        if not flag:
            raise ValueError
        if type == "for":
            resp, all_states = await self.for_executor(content, all_states)
        elif type == "遍历":
            resp, all_states = await self.chinese_executor(content, all_states)

        return resp, all_states
...
END
        ...
        if len(matches) == 2:
            ...
            for item in iter_list_values:
                self.local_value = item
                await self.segment_executor(for_content, all_states)
        else:
            pass
        return "", all_states
